...
class Maze:
    ...
    def initialize_pheromones(self):
        self.pheromones = copy.deepcopy(self.walls)     # start value 0 for inaccessible and 1 for accessible
    ...
    def add_pheromone_route(self, route, q):
        if (route is None):
            return
        pheromone_amount = q / route.size()
        current_position = route.start

        # Next line is for adding pheromones to starting position, which we probably don't want to
        # self.add_pheromone(current_position, pheromone_amount)

        for dir in route.route:
            current_position = current_position.add_direction(dir)
            self.add_pheromone(current_position, pheromone_amount)
    ...
    def evaporate(self, rho):
       for x in range(self.width):
           for y in range(self.length):
               self.pheromones[x][y] = (1 - rho) * self.pheromones[x][y]
    ...
    def get_surrounding_pheromone(self, position):
        pheromone_east = self.get_pheromone(position.add_direction(Direction.east))
        pheromone_north = self.get_pheromone(position.add_direction(Direction.north))
        pheromone_west = self.get_pheromone(position.add_direction(Direction.west))
        pheromone_south = self.get_pheromone(position.add_direction(Direction.south))
        return [pheromone_east, pheromone_north, pheromone_west, pheromone_south]
    ...
    # @param pos Position coordinate
    # @return pheromone at point
    def add_pheromone(self, pos, pheromone_amount):
        if self.in_bounds(pos):
            self.pheromones[pos.get_x()][pos.get_y()] += pheromone_amount

    # Pheromone changer for a specific position.
    # @param pos Position coordinate
    def change_pheromone(self, pos, pheromone_amount):
        if self.in_bounds(pos):
            self.pheromones[pos.get_x()][pos.get_y()] = pheromone_amount
